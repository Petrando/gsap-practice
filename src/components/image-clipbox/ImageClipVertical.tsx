...
const ImageClipVertical: FC<Props> = ({ parentRef, src, clipClass = "", slices = 8 }) => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      slicesEl.forEach((slice, i) => {
        ...
        // OUT animation (reverse direction)
        gsap.to(slice, {
          y: toY,
          opacity: 0,
          duration: 0.8,
    ...
    <div ref={containerRef} className={`relative ${clipClass}`}>
      ...
      <div className="absolute left-0 top-0 overflow-hidden" style={{ width, height }}>
        {Array.from({ length: slices }).map((_, i) => (
          <div
            key={i}
            className="absolute top-0 h-full slice bg-cover bg-center"
            style={{
              left: `${i * sliceWidth}px`,
              width: `${sliceWidth}px`,
              backgroundImage: `url(${src})`,
              backgroundSize: `${width}px ${height}px`,
              backgroundPosition: `-${i * sliceWidth}px 0px`,
            }}
          />
        ))}
      </div>
...
export default ImageClipVertical;
